 ...
 * 
 * Self-contained pipeline infrastructure for the notification-domain domain:
...
export interface NotificationDomainPipelineStackProps extends cdk.StackProps {
  domain: string;
...
export class NotificationDomainPipelineStack extends cdk.Stack {
  public readonly artifactBucket: s3.Bucket;

  constructor(scope: Construct, id: string, props: NotificationDomainPipelineStackProps) {
      ...
      post: [
        new pipelines.CodeBuildStep(`Deploy${this.pascalCase(domain)}Mimic`, {
          commands: [
            ...
            'npm ci --no-audit --no-fund',
            'npm run build',
            'npx cdk deploy "mimic-use1-hand-made-*-stack" --require-approval never',
      ...
      post: [
        new pipelines.CodeBuildStep(`Deploy${this.pascalCase(domain)}Prod`, {
          commands: [
            ...
            'npm ci --no-audit --no-fund',
            'npm run build',
            'npx cdk deploy "prod-use1-hand-made-*-stack" --require-approval never',
