...
export const handler = async (...args: unknown[]): Promise<void> => {
  ...
  initTelemetryLogger(event, { domain: "notification-domain", service: "republish" });
  console.log("REPUBLISH LAMBDA START", {domain: DOMAIN_NAME, eventBus: EVENT_BUS_NAME});
  if (!OUTBOX_TABLE_NAME || !EVENT_BUS_NAME || !DOMAIN_NAME) throw new Error("Missing env vars");
  ...
  try {
    ...
    console.log(`Republish complete. Success: ${success}, Failure: ${failure}`);
  } catch (err) {
